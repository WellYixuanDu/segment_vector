...
public:
    ...
    segment_vector_iterator& operator=(const segment_vector_iterator& other) noexcept
    {
    ...
    friend bool operator!=(const _Self& x, const segment_vector_iterator<ValueType, RefR, PtrR, N>& y) noexcept
    {
        return !(x == y);
    }
    ...
    friend bool operator<(const _Self& x, const _Self& y) noexcept
    {
       return (x.m_node == y.m_node)
            ? (x.m_cur < y.m_cur) : (x.m_node < y.m_node);
    }
    template<typename RefR, typename PtrR>
    friend bool operator<(const _Self& x, const segment_vector_iterator<ValueType, RefR, PtrR, N>& y) noexcept
    {
    ...
    template<typename RefR, typename PtrR>
    friend difference_type operator-(const _Self& x, const segment_vector_iterator<value_type, RefR, PtrR, N>& y) noexcept
    {
        return difference_type(buffer_size())
            * (x.m_node - y.m_node - int(x.m_node != 0))
            + (x.m_cur - x.m_first)
...
// blocksize 字节
template<typename _Tp, typename _Alloc = std::allocator<_Tp>, uint32_t _BlockSize = 4096>
...
private:
    static constexpr size_t buffer_size() noexcept
    {
        return (sizeof(_Tp) < _BlockSize ? size_t(_BlockSize) / sizeof(_Tp) : size_t(1));
    }

public:

    using iterator = segment_vector_iterator<
                        _Tp,
                        _Tp&,
                        pointer,
                        buffer_size()
    ...
    using const_iterator = segment_vector_iterator<
                        _Tp,
                        const _Tp&,
                        const_pointer,
                        buffer_size()
...
public:
    ...
    allocator_type& get_Tp_allocator() noexcept
    {
        return m_allocator;
    }
...
public:
    ...
    // 构造函数 check
    explicit
    segment_vector(const allocator_type& alloc = allocator_type())
    ...
    explicit
    segment_vector(size_type n, const allocator_type& alloc = allocator_type())
    : m_allocator(_Tp_alloc_type(alloc))
    , m_start()
    , m_finish()
    , m_map(nullptr)
    ...
    segment_vector(size_type n, const _Tp& value, const allocator_type& alloc = allocator_type())
    : m_allocator(_Tp_alloc_type(alloc))
    , m_start()
    , m_finish()
    , m_map(nullptr)
    ...
    : m_allocator(_Alloc_traits::select_on_container_copy_construction(other.get_Tp_allocator()))
    {
    ...
    segment_vector(std::initializer_list<value_type> l, const allocator_type& alloc = allocator_type())
    : m_allocator(_Tp_alloc_type(alloc))
    {
    ...
    segment_vector(InputIterator first, InputIterator last, const allocator_type& alloc = allocator_type())
    : m_allocator(_Tp_alloc_type(alloc))
    {
...
public:
    ...
    {
        STL_ASSERT(_Alloc_traits::propagate_on_container_swap::value || get_Tp_allocator() == x.get_Tp_allocator());
        if (this == &x)
        {
            return;
        }
...
public:
    ...
    */
    ...
    iterator insert(const_iterator position, std::initializer_list<value_type> l)
    {
        ...
        range_insert_aux(position._M_const_cast(), l.begin(), l.end(), std::random_access_iterator_tag());
        return begin() + offset;
    }
    // 在指定位置 插入 n 个 x元素
    ...
    {
        difference_type offset = position - cbegin();
        range_insert_aux(position._M_const_cast(), first, last, std::__iterator_category(first));
        return begin() + offset;
    }
    ...
    void emplace_back(Args&&... args)
    {
        ...
        {
            _Alloc_traits::construct(get_Tp_allocator(), this->m_finish.m_cur, std::forward<Args>(args)...);
            ++this->m_finish.m_cur;
        }
        else
        {
...
public:
    ...
    segment_vector& operator=(segment_vector& x)
    {
        if (&x != this)
        {
            ...
            else
            {
                ...
                range_insert_aux(this->m_finish, mid, x.end(), std::random_access_iterator_tag());
            }
        }
        return *this;
    }
    ...
    void shrink_to_fit()
    {
        ...
        const size_type new_table_capacity = std::max(min_table_capacity(), new_map_size);
        ...
        create_nodes(m_start.m_node + new_map_size, m_start.m_node + new_table_capacity);
...
private:
    ...
    void initialize_fill_n(size_type n, const _Tp& value)
    {
        ...
        STL_TRY
        {
            for (cur = m_start.m_node; cur < m_finish.m_node; ++cur)
            {
                std::__uninitialized_fill_n_a(*cur, buffer_size(), value, get_Tp_allocator());
            }
            std::__uninitialized_fill_n_a(m_finish.m_first, n % buffer_size(), value, get_Tp_allocator());
        }
        STL_CATCH (...)
        {
            destroy_storage();
    ...
    void initialize_copy(const segment_vector& other)
    {
        ...
        STL_TRY
        {
            std::__uninitialized_copy_a(other.m_start, other.m_finish, this->m_start, get_Tp_allocator());
        }
        STL_CATCH (...)
        {
            destroy_storage();
    ...
    void initialize_move(segment_vector& other)
    {
        ...
        else
        {
            ...
            STL_TRY
            {
                std::__uninitialized_move_a(other.m_start, other.m_finish, m_start, get_Tp_allocator());
            }
            STL_CATCH (...)
            {
                destroy_storage();
    ...
    void initialize_range(InputIterator first, InputIterator last, std::input_iterator_tag)
    {
        allocate_map_and_nodes(0);
        STL_TRY
        {
    ...
    void initialize_range(ForwardIterator first, ForwardIterator last, std::forward_iterator_tag)
    {
        ...
        for (cur_node = this->m_start.m_node; cur_node < this->m_finish.m_node; ++cur_node)
        {
        ...
        std::__uninitialized_copy_a(first, last, this->m_finish.m_first, get_Tp_allocator());
    }
...
private:
    ...
    */
    ...
    void reallocate_map(size_type nodes_to_add)
    {
        ...
        const size_type old_num_nodes = this->m_finish.m_node - this->m_start.m_node + 1;
        //const size_type new_num_nodes = old_num_nodes + nodes_to_add;

        map_pointer new_nstart;
        size_type new_map_size = this->m_map_size + std::max(this->m_map_size, nodes_to_add) + 1;
        map_pointer new_map = this->allocate_map(new_map_size);
        new_nstart = new_map;
        std::copy(this->m_start.m_node, this->m_start.m_node + m_map_size, new_nstart);
        ...
        create_nodes(this->m_start.m_node + m_map_size, this->m_start.m_node + new_map_size);
...
private:
    ...
    void assign_move(segment_vector& x)
    {
        ...
        {
            assign_aux(std::make_move_iterator(x.m_start), std::make_move_iterator(x.m_finish), std::random_access_iterator_tag());
        }
    }
...
private:
    ...
    void range_check(size_type n) const
    {
        if (n >= this->size())
        {
            throw std::out_of_range("segment_vector::range_check: n: >= this->size():");
        }        
    }
    
    // 最小segment数量
    static constexpr size_type min_table_capacity()
    {
        return 16;
    }
    ...
    {
        const size_type vacancies = this->m_finish.m_last - this->m_finish.m_cur - 1;
        if (n > vacancies)
        {
    ...
    // 更换数据
    void swap_data(segment_vector& x) noexcept
    {
        std::swap(*this, x);
    }
    // 析构并清空内存后交换数据 
...
private:
    ...
    */
    ...
    void push_back_aux(Args&&... args)
    {
        ...
        STL_TRY
        {
            _Alloc_traits::construct(get_Tp_allocator(), m_finish.m_cur, std::forward<Args>(args)...);
    ...
    void insert_aux(iterator pos, ForwardIterator first, ForwardIterator last, size_type n)
    {
        ...
        STL_TRY
        {
            if (elems_after > difference_type(n))
            {
                ...
                std::__uninitialized_move_a(finish_n, this->m_finish, this->m_finish, get_Tp_allocator());
            ...
            else
            {
                ...
                std::__uninitialized_copy_move(mid, last, pos, this->m_finish, this->m_finish, get_Tp_allocator());
                this->m_finish = new_finish;
    ...
    void insert_aux(iterator pos, size_type n, const value_type& x)
    {
        ...
        STL_TRY
        {
            ...
            {
                std::__uninitialized_fill_move(this->m_finish, pos + difference_type(n), x_copy, pos, this->m_finish, get_Tp_allocator());
                this->m_finish - new_finish;
    ...
    template<typename InputIterator>
    void range_insert_aux(iterator pos, InputIterator first, InputIterator last, std::input_iterator_tag)
    {
        if (pos == end())
        {
            for (; first != last; ++first)
    ...
    // 在pos位置插入从 first 到 last 的元素
    template<typename ForwardIterator>
    void range_insert_aux(iterator pos, ForwardIterator first, ForwardIterator last, std::forward_iterator_tag)
    {
        const size_type n = std::distance(first, last);
        if (pos.m_cur == this->m_finish.m_cur)
        {
            ...
            STL_TRY
            {
                std::__uninitialized_copy_a(first, last, this->m_finish, get_Tp_allocator());
                this->m_finish = new_finish;
            }
            STL_CATCH (...)
            {
    ...
    void fill_insert(iterator pos, size_type n, const value_type& x)
    {
        ...
        {
            iterator new_finish = reserve_elements_at_back(n);
            std::__uninitialized_fill_a(this->m_finish, new_finish, x, get_Tp_allocator());
            this->m_finish = new_finish;
        }
        else
        {
    ...
    void default_append(size_type n)
    {
        ...
        {
            iterator new_finish = reserve_elements_at_back(n);
            std::__uninitialized_default_a(this->m_finish, new_finish, get_Tp_allocator());
            this->m_finish = new_finish;
        }
    }
...
inline bool operator==(segment_vector<_Tp, _Alloc, _BlockSize>& x, segment_vector<_Tp, _Alloc, _BlockSize>& y)
{
...
template<typename _Tp, typename _Alloc, uint32_t _BlockSize>
inline bool
operator<=(segment_vector<_Tp, _Alloc, _BlockSize>& x, segment_vector<_Tp, _Alloc, _BlockSize>& y)
{ return !(y < x); }     
...
template<typename _Tp, typename _Alloc, uint32_t _BlockSize>
inline void swap(segment_vector<_Tp, _Alloc, _BlockSize>& x, segment_vector<_Tp, _Alloc, _BlockSize>& y) _GLIBCXX_NOEXCEPT_IF(noexcept(x.swap(y)))
{ x.swap(y); }     
